...
$durationsByDate = collect($dateGroups)->mapWithKeys(function ($dateGroup) use ($activities, $projectIds) {
    $groupedActivities = $activities->filter(function ($activity) use ($dateGroup) {
        $activityDate = Carbon::parse($activity->start_time);
        return $activityDate->between($dateGroup['start'], $dateGroup['end']);
    });

    $projectDurations = $groupedActivities->groupBy('project_id')->map(function ($projectGroup) {
        return $projectGroup->sum(function ($activity) {
            return durationToSeconds($activity->durations); // Use the single function declaration
        });
    });

    return [$dateGroup['label'] => $projectDurations];
})->toArray();
// Prepare chart data
$chartData = [
                    ...
                    // Process activities for detailed data
                     ...
                     $id = $activity->id ?? null;
                        ...
                        $activity_data[] = [
                            ...
                            'mouse_clicks'=>$activity->mouse_click,
                            'keyboard_hits'=>$activity->keyboard_click,
...
foreach ($projectIds as $projectId) {
    ...
    foreach ($memberIds as $memberId) {
        if (!isset($memberDurations[$memberId])) {
            $memberDurations[$memberId] = array_fill_keys($dateRanges, 0);
            $projectDetails[$memberId] = [];
        }
        ...
        foreach ($activities as $activity) {
            ...
            foreach ($dateRanges as $range) {
                ...
                // Adjust for year, month, and day comparison
                if ($activityStartDate->between($rangeStart, $rangeEnd->endOfDay()) || 
                    $activityEndDate->between($rangeStart, $rangeEnd->endOfDay())) {
                    ...
                    if (!isset($projectDetails[$memberId][$activity->title])) {
                        $projectDetails[$memberId][$activity->title] = [
                            ...
                            'daily_durations' => array_fill_keys($dateRanges, 0),
                            'total_duration' => 0,
                        ];
                    }
